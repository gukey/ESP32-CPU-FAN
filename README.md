...
上电时输出 0%，只有收到本次连接的合法温度数据后才启动；普通启动暂用 30% 并保持至少 1 秒后缓降，高温直接全速。30% 为试用启动值，无转速反馈，需根据实际风扇调整源码中的 `startupDutyPercent`。CPU/GPU 分别在 12 秒未更新后失效，使用仍有效的较高温度；两路都失效则停转并关闭屏幕。
