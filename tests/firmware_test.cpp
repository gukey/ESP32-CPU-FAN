...
int main(){
 ...
 assert(dutyCycle==71 && recoveryArmed && SerialBT.begins==1);
 ...
 assert(cpuValue==65 && dutyCycle==31);
 ...
 send("CPU60\n");assert(dutyCycle==23 && recoveryArmed);
 uint32_t cpuSample=cpuTime;
 testTime=cpuSample+11000;send("GPU50\n");
 testTime=cpuSample+12000;loop();assert(!cpuValid && gpuValid && dutyCycle==15);
